  ...
  return (
    <Card>
      <CardHeader className='border-b'>
        <h2 className='text-xl font-semibold'>{mode === 'store' ? 'Resumen del pedido' : 'Resumen de cotización'}</h2>
      </CardHeader>
        ...
        {items.map((item) => (
          <div key={item.id} className='flex gap-4'>
            ...
            <div className='flex-1'>
              ...
              {mode === 'store' && <p className='text-primary font-bold'>${(item.price * item.quantity).toFixed(2)}</p>}
            </div>
          </div>
        ))}
